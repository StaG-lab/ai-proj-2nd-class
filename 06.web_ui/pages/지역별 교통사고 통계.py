...
df = pd.read_csv('C:/Users/USER/Documents/ai-proj-2nd-class/04.data_preprocess/2nd-dataset_경기 수원시_20250530_104914.csv')

tabs = st.tabs(["수원", "송파", "지역검색"])
...
with tabs[0]:
    ...
    # 각 종목별 내용
    if menu == "1. Initial Data Inspection":
    ...
    elif menu == "2. Data Cleaning and Preprocessing":
        ...
        # Clean sport_types: "없음" means no sport, otherwise it's a list
        # For analysis, we can create dummy variables for common sports
        all_sports = set()
        for sports_list in df['sport_types'].unique():
            if sports_list != "없음":
        ...
        for sport in sorted(list(all_sports)):
            df[f'is_{sport}'] = df['sport_types'].apply(lambda x: sport in x)

        st.write("\nDataFrame with new sport boolean columns (first 5 rows):")
        st.write(df.head())

    elif menu == "3. Feature Engineering (Time-based features)":
        st.header("3. Feature Engineering (Time-based features)")
    ...
    elif menu == "4. Univariate Analysis":
        st.header("\n4. Univariate Analysis")
    ...
    elif menu == "5. Bivariate and Multivariate Analysis":
        ...
        ax.set_ylabel('Accident Count')
        ax.set_xticklabels([0, 1], ['No Game', 'Game'])
        ax.grid(True)
        st.pyplot(fig)
        ...
        ax.set_ylabel('Accident Count')
        ax.set_xticklabels([0, 1], ['Regular Game', 'Playoff Game'])
        ax.grid(True)
        st.pyplot(fig)
        ...
        ax.set_ylabel('Accident Count')
        ax.set_xticklabels([0, 1], ['Non-Holiday', 'Holiday'])
        ax.grid(True)
        st.pyplot(fig)
    ...
    elif menu == "6. Time Series Analysis of Accident Counts":
        st.header("6. Time Series Analysis of Accident Counts")
    ...
    elif menu == "7. Combined Effects (Examples)":
        st.header("7. Combined Effects (Examples)")
        ...
        ax.set_xlabel('Was there a game?')
        ax.set_xticklabels([0,1], ['No Game', 'Game Day'])
        ax.set_ylabel('Average Accident Count')
        ...
        st.write(high_accident_days[['date', 'accident_count', 'game_count', 'sport_types', 'has_playoff', 'temperature', 'precipitation', 'snow_depth', 'weather_condition', 'is_holiday', 'weekday_final']].sort_values(by='accident_count', ascending=False))
...
with tabs[1]:
    st.header("송파 통계")
    st.write("송파파 관련 교통사고 통계 내용을 여기에 작성")
...
with tabs[2]:
    ...
    # DB 연결
    with open("C:/Users/USER/Documents/ai-proj-2nd-class/03.db_code/db_config.json", "r") as f:
        config = json.load(f)
    ...
    st.write("--- 1. Initial Data Inspection ---")
    st.write("Shape of the dataframe:", df.shape)
    st.write("\nFirst 5 rows:")
    st.write(df.head())
    st.write("\nData types and non-null counts:")
